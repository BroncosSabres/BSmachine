// nhl-distribution-chart.js
// NHL margin/total-goals distribution modal — machine (simulation) curve only.
// Direct port of nfl-distribution-chart.js: same PDF/CDF toggle, CSV export,
// and 3D joint-distribution toggle shared via distribution-3d.js, with
// "points" swapped for "goals" throughout.
...
async function fetchDistribution(gameId) {
  ...
  try {
    const res  = await fetch(apiUrl('nhl', `game_score_distributions/${gameId}`));
...
function ensureModal() {
  if (document.getElementById('nhl-dist-modal')) return;
  const el = document.createElement('div');
  el.id = 'nhl-dist-modal';
  ...
  el.innerHTML = `
    <div style="background:#161b24;border:1px solid #2e3a4e;border-radius:16px;width:100%;max-width:min(900px,calc(100vw - 2rem));max-height:90vh;overflow-y:auto;padding:1.5rem;position:relative;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;">
        <div id="nhl-dist-modal-title" style="font-family:'Barlow Condensed',system-ui,sans-serif;font-size:1.1rem;font-weight:700;color:#e2e8f0;"></div>
        <button id="nhl-dist-modal-close" style="background:none;border:none;color:#4a5568;cursor:pointer;font-size:1.25rem;line-height:1;padding:0.25rem;">✕</button>
      </div>
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.25rem;flex-wrap:wrap;gap:0.75rem;">
        <div style="display:flex;align-items:center;gap:1.25rem;font-size:0.75rem;color:#4a5568;">
          ...
          <span id="nhl-dist-modal-result-legend" style="display:none;align-items:center;gap:5px;">
        ...
        <div style="display:flex;align-items:center;gap:0.75rem;">
          <div id="nhl-dist-mode-toggle" style="display:flex;gap:2px;background:#0f1117;border:1px solid #2e3a4e;border-radius:6px;padding:2px;">
            <button id="nhl-dist-toggle-pdf" style="padding:3px 10px;border-radius:4px;border:none;font-size:0.7rem;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;cursor:pointer;background:#f59e0b;color:#0a0d14;">PDF</button>
            <button id="nhl-dist-toggle-cdf" style="padding:3px 10px;border-radius:4px;border:none;font-size:0.7rem;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;cursor:pointer;background:transparent;color:#4a5568;">CDF</button>
            <button id="nhl-dist-toggle-3d" style="padding:3px 10px;border-radius:4px;border:none;font-size:0.7rem;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;cursor:pointer;background:transparent;color:#4a5568;">3D</button>
          </div>
          <button id="nhl-dist-download-csv" style="padding:3px 10px;border-radius:4px;border:1px solid #2e3a4e;font-size:0.7rem;font-weight:700;letter-spacing:0.04em;text-transform:uppercase;cursor:pointer;background:transparent;color:#4a5568;" title="Download distribution data as CSV">&#8595; CSV</button>
        </div>
      </div>
      <div id="nhl-dist-modal-loading" style="text-align:center;color:#4a5568;font-size:0.875rem;padding:2rem;">Loading distributions…</div>
      <div id="nhl-dist-modal-charts" style="display:none;">
        <div style="margin-bottom:1.5rem;">
          ...
          <canvas id="nhl-dist-modal-margin"></canvas>
        </div>
        <div>
          <div style="font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.06em;color:#4a5568;margin-bottom:0.5rem;">Total Goals</div>
          <canvas id="nhl-dist-modal-total"></canvas>
        </div>
      </div>
      <div id="nhl-dist-modal-3d" style="display:none;height:520px;"></div>
    </div>
  `;
  document.body.appendChild(el);

  el.addEventListener('click', e => { if (e.target === el) closeModal(); });
  document.getElementById('nhl-dist-modal-close').addEventListener('click', closeModal);
  document.addEventListener('keydown', e => { if (e.key === 'Escape') closeModal(); });
}

function closeModal() {
  const modal = document.getElementById('nhl-dist-modal');
  if (modal) modal.style.display = 'none';
  const tt = document.getElementById('nhl-dist-chart-tooltip');
  if (tt) tt.style.display = 'none';
  chartInstances['margin']?.destroy(); delete chartInstances['margin'];
  chartInstances['total']?.destroy();  delete chartInstances['total'];
  const el3d = document.getElementById('nhl-dist-modal-3d');
  if (el3d) purgeDistribution3D(el3d);
}

function ensureTooltipEl() {
  let el = document.getElementById('nhl-dist-chart-tooltip');
  if (!el) {
    el = document.createElement('div');
    el.id = 'nhl-dist-chart-tooltip';
    el.style.cssText = `
...
const hideTooltip = () => { const el = document.getElementById('nhl-dist-chart-tooltip'); if (el) el.style.display = 'none'; };

export async function openDistModal(title, gameId, actualMargin = null, actualTotal = null) {
  ensureModal();
  const modal   = document.getElementById('nhl-dist-modal');
  const loading = document.getElementById('nhl-dist-modal-loading');
  const charts  = document.getElementById('nhl-dist-modal-charts');
  const dist3d  = document.getElementById('nhl-dist-modal-3d');
  const titleEl = document.getElementById('nhl-dist-modal-title');
  ...
  const resultLegend = document.getElementById('nhl-dist-modal-result-legend');
  if (resultLegend) resultLegend.style.display = actualMargin !== null ? 'flex' : 'none';

  const machineMBins = machineDist?.margins ? normaliseMachineBins(machineDist.margins, 1, -20, 20) : [];
  const machineTBins = machineDist?.totals  ? normaliseMachineBins(machineDist.totals,  1,   0, 20) : [];

  const csvBtn = document.getElementById('nhl-dist-download-csv');
  if (csvBtn) {
    csvBtn.onclick = () => {
      ...
      rows.push('');
      rows.push('Total Goals,BS Machine');
  ...
  function renderCharts(mode) {
    ...
    const mCtx = document.getElementById('nhl-dist-modal-margin')?.getContext('2d');
    if (mCtx) {
      chartInstances['margin'] = new Chart(mCtx, {
        data: { datasets: makeDatasets(machineMBins, actualMargin, mode) },
        options: makeChartOptions('Margin (home – away, goals)', machineMBins, marginLabelFn, mode, -20, 20),
    ...
    const tCtx = document.getElementById('nhl-dist-modal-total')?.getContext('2d');
    if (tCtx) {
      chartInstances['total'] = new Chart(tCtx, {
        data: { datasets: makeDatasets(machineTBins, actualTotal, mode) },
        options: makeChartOptions('Total goals', machineTBins, totalLabelFn, mode, 0, 20, 1),
        plugins: [crosshairPlugin],
      });
  ...
  const pdfBtn = document.getElementById('nhl-dist-toggle-pdf');
  const cdfBtn = document.getElementById('nhl-dist-toggle-cdf');
  const btn3d  = document.getElementById('nhl-dist-toggle-3d');
  ...
  function setMode(mode) {
    ...
    if (mode === '3d') {
      ...
      renderDistribution3D(dist3d, { sport: 'nhl', id: gameId });
    } else {
      dist3d.style.display = 'none';
